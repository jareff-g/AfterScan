...
class JobEntry:
    ...
    # CRITICAL: Use default_factory for new field
    job_id: str = field(default_factory=lambda: str(uuid.uuid4()))
    
    # Nested dataclass: Configuration for the project being processed
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'JobEntry':
        """
        Creates a JobEntry instance from a dictionary, handling legacy files 
        that may be missing the 'job_id' field.
        """
        valid_fields = {f.name for f in fields(cls)}
        
        # 1. Filter the incoming data to only include valid fields
        filtered_data = {key: value for key, value in data.items() if key in valid_fields}

        # 2. Handle the nested 'project_config' field manually
        # This must be done because the value in 'data' is a dictionary, 
        # but the JobEntry expects a ProjectConfigEntry instance.
        if 'project_config' in filtered_data:
            # CRITICAL: Call the ProjectConfigEntry's own from_dict method here!
            pc_data = filtered_data.pop('project_config')
            filtered_data['project_config'] = ProjectConfigEntry.from_dict(pc_data)
        
        # 3. Handle the missing 'job_id' (Backward Compatibility)
        # If 'job_id' is NOT in the filtered data (e.g., loaded from a legacy file), 
        # it will be automatically assigned a unique UUID by the dataclass default.
        # If 'job_id' IS present (from a new file), it will use the existing value.
...
class JobList:
    ...
    current_job_name: Optional[str] = field(default=None)
    
    @classmethod
    def from_dict(cls, data: Dict[str, Dict[str, Any]]) -> 'JobQueue':
        """
        Constructs the JobQueue from the top-level dictionary.
        
        ASSUMPTION: The ConfigurationManager has migrated the keys of the 
        nested job data (the values) to snake_case.
        """
        instance = cls()
        
        for job_name, job_data in data.items():
            try:
                # job_name (key) is always treated as a string
                job_entry = JobEntry.from_dict(job_data, job_name_key=job_name)
                instance.job_map[job_name] = job_entry
                
            except Exception as e:
                logging.error(f"Failed to load job '{job_name}': {e}")
                continue
    ...
    def to_dict(self) -> Dict[str, Dict[str, Any]]:
        """
        Converts the internal job_map back into the file structure 
        using job names as keys and snake_case data as values.
        """
        output_data: Dict[str, Dict[str, Any]] = {}
        
        for job_name, job_entry in self.job_map.items():
            # Delegate serialization to JobEntry, preserving job_name as the key
    ...
    def add_job(self, job_name: str, job: JobEntry):
    ...
    def get_next_job(self) -> Optional[JobEntry]:
        """Simple logic to get the next job that is not 'done'."""
        for job in self.job_map.values():
            if not job.done:
                self.current_job_name = job.job_name
                return job
        return None
'''